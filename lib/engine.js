...
import { generateTypst } from './typst.js';
...
class Context {
  ...
  theme (name, options) {
    this.#execution.push(async () => {
      ...
      this.#data.theme = { name, options };
      await theme(options, this, this.#data);
    });
    return this;
  }
  ...
  typst (outTypst, outPDF) {
    this.#execution.push(async () => {
      await generateTypst(this.#data, absolutise(outTypst), absolutise(outPDF), this.#currentDir);
    });
    return this;
  }
