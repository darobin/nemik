...
import { googleAuth, regenerateGoogleToken, isGoogleAuthError, isGoogleAPIDisabledError, apiDisabledError } from "./credentials.js";
...
export class GDocSource {
  ...
  async doc (src) {
    ...
    try {
      return await this.#fetch(src);
    }
    catch (err) {
      if (isGoogleAPIDisabledError(err)) throw apiDisabledError(err);
      if (!isGoogleAuthError(err)) throw err;
      // The refresh token is gone or revoked (Google does that after a week for apps in
      // "Testing" status): get a new one and try once more.
      console.warn(`Google authorisation for "${this.#service}" is no longer valid, requesting a new one.`);
      await regenerateGoogleToken(this.#service);
      await this.auth(this.#service);
      return await this.#fetch(src);
    }
  }
  async #fetch (src) {
...
class HTMLConverter {
  ...
  async tabs (tabs, parent) {
    ...
    for (const { tabProperties, documentTab: { body, lists, footnotes, inlineObjects }, childTabs } of tabs) {
      this.#footnoteOrder = [];
      this.#lists = lists;
      const tab = el('div', { id: tabProperties.tabId, title: tabProperties.title, class: 'tab' }, [], parent);
  ...
  async paragraph (paragraph, parent) {
    const { el, txt } = htmlHelpers(this.#doc);
    if (hasOnlyOne('horizontalRule', paragraph.elements)) {
      ...
      if (/^HEADING_/.test(type)) {
        ...
        if (end.nodeType === 3 && /\s+$/.test(end.nodeValue)) {
          text[text.length - 1] = txt(end.nodeValue.replace(/\s+$/, ''));
        }
        ...
        const attrs = {};
        if (paragraph.paragraphStyle.alignment === 'CENTER') {
          attrs.style = 'text-align: center';
        }
        el('p', attrs, text, parent);
      }
    }
  }
  ...
  async footnotes (footnotes, parent) {
    const { el } = htmlHelpers(this.#doc);
    const section = el('section', { role: 'doc-endnotes', class: 'special-section' }, [], parent);
    let idx = 0;
