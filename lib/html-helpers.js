...
export default function htmlHelpers (doc) {
  ...
  // Assumes sections, gets appendix title and makes those sections appendices
  function appendixify (appendices) {
    const titles = new Set(appendices);
    // a document that starts at h2 has its top-level sections under h2
    const hasH1 = [...doc.querySelectorAll('h1')].some(h => !h.closest('header, .special-section'));
    [...doc.querySelectorAll(hasH1 ? 'h1' : 'h2')]
      .filter(h => titles.has(h.textContent.trim()))
      .forEach(h => h.parentNode.classList.add('special-section', 'appendix'))
    ;
  }

  function footnotify () {
    const sec = doc.querySelector('section[role="doc-endnotes"]');
    if (!sec) return;
    const h1 = el('h1', {}, ['Footnotes']);
    sec.prepend(h1);
    const ol = el('ol', {}, [], sec);
    [...sec.querySelectorAll('aside')].forEach(aside => {
      aside.querySelector('a').remove();
      const li = el('li', { id: aside.getAttribute('id'), role: 'doc-endnote' }, [], ol);
      while (aside.hasChildNodes()) li.append(aside.firstChild);
      aside.remove();
    });
    appendixify(['Footnotes']);
  }

  function lang (ln) {
    doc.querySelector('html').setAttribute('lang', ln);
  }
