...
export class Zotero {
  ...
  insertBibliography () {
    ...
    this.refIDs.forEach(id => {
      ...
      while (link.nextSibling?.localName === 'a' && link.nextSibling.getAttribute('href') === link.getAttribute('href')) {
        link.textContent += link.nextSibling.textContent;
        link.nextSibling.remove();
      }
      const data = this.biblioData[id];
      let refs = link.textContent.replace(/^\s*\(\s*/, '').replace(/\s*\)\s*$/, '').split(/\s*;\s*/);
      const replace = [];
      refs.forEach(r => {
        if (refs.find(r => /(,\s+\d{4}(?:[a-z]+)?)+$/.test(r))) {
          const parts = r.split(/\s*,\s*/);
          const name = parts[0].replace(/(^.*)\s+\d{4}(?:[a-z]+)?/, '$1');
          parts[0] = parts[0].replace(name, '');
          parts.forEach(p => {
            if (!/\s*\d{4}(?:[a-z]+)?\s*/.test(p)) return;
            replace.push(`${name} ${p}`);
          });
        }
        else {
      ...
      refs.forEach((ref, idx) => {
        ...
        // NOTE: removed the code to merge refs because I think it's incorrect
        // const mergedRef = ref.replace(/\D+$/, ''); // in case there are subscripts in tab
        const mergedRef = ref;
        // if (/berjon/i.test(ref)) console.warn(`merged=${mergedRef}, ref=${ref}, id=${items[idx].id}`);
    ...
    params.entry_ids.forEach((id, idx) => {
      // console.warn(`MAP id=${id}, nemikRef=${this.citedItems[id].nemikRef}`);
