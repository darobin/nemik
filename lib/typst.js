...
// XXX
//  div: some classes missing
//  a biblioref # -> bookmark (but do the bookmarking first)
const CLOSE = `"")+`;
const CLOSE_ITEM = `""),`;
const CLOSE_ITEMS = ') +';
  ...
  switch (n) {
    case "html":
      out.push(
        ...
        'set page(paper: "a4")',
        [
          `set document(title: "${esc(el.ownerDocument.title)}"`,
          `${theme?.options?.author ? `, author: "${esc(theme.options.author)}"` : ""}`,
          `${theme?.options?.date ? `, date: ${datetime(theme.options.date)}` : ""}`,
          `)`
        ].join(''),
        `let body(b) = { block(b) }`,
        `let main(b) = { block(b) }`,
        `let header(b) = { block(b) }`,
        `let section(b) = { block(b) }`,
    ...
    case "u":
      ...
      break;
    case "a":
      out.push(`link("${esc(el.getAttribute('href'))}", `);
      walkChildren(el, ctx);
      out.push(CLOSE);
      break;
    case "span":
    ...
    case "img":
      ...
      break;
    case "table":
      out.push(`table(columns: ${el.querySelector('tr')?.querySelectorAll('td')?.length || 1}, `);
      walkChildren(el, ctx);
