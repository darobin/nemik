...
export function renderBiblio (data) {
  const { document } = data;
  if (!data.references || !Object.keys(data.references).length) return;
  const { el } = htmlHelpers(document);
  const refEls = [];
  // XXX NOT AN OBJECT A SET
  // XXX NEED TO GET THE ACTUAL REFS
  Object.keys(data.references).sort().forEach(k => {
    const v = data.references[k];
